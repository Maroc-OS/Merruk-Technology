...
#ifdef CONFIG_BACKLIGHT_S2C
struct platform_device bcm_s2c_backlight_devices = {
	.name = "s2c-backlight", 
	.id = 0,
};
static struct platform_s2c_backlight_data bcm_s2c_backlight_data = {
	.max_brightness = 32,
	.dft_brightness = 28,
	.ctrl_pin = 17,
};
...
#if defined(CONFIG_TOUCHSCREEN_MMS128) || defined(CONFIG_TOUCHSCREEN_MMS128_REV04)
	{
...
#ifdef CONFIG_TOUCHSCREEN_CYTTSP_I2C
	{
				I2C_BOARD_INFO(CY_I2C_NAME, 0x20),
...
int board_sysconfig(uint32_t module, uint32_t op)
{
	...
	case SYSCFG_HEADSET:
		...
		if (op == SYSCFG_INIT){
			...
			//writel(readl(ADDR_SYSCFG_ANACR2) | SYSCFG_ANACR2_AUDIO_RXLDO_PWRDOWN, ADDR_SYSCFG_ANACR2);
			writel(readl(HW_IRQ_BASE + 0x0008) | 0x1000, (HW_IRQ_BASE + 0x0008));//clear bit 12 MICON
		}
...
#endif
#ifdef CONFIG_BACKLIGHT_S2C
	bcm_s2c_backlight_devices.dev.platform_data = &bcm_s2c_backlight_data;
#endif
#ifdef CONFIG_KEYBOARD_BCM
