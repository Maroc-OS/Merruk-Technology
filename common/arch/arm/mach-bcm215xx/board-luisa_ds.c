...
*
*	@file	arch/arm/mach-bcm215xx/board-luisa-ds.c
*
...
#endif
#if defined(CONFIG_KEYBOARD_BCM) || defined(CONFIG_INPUT_GPIOKEY_MATRIX)
...
#define BATT_LEVEL5_VOLT	3941
#define BATT_LEVEL4_VOLT	3845
#define BATT_LEVEL3_VOLT	3770
#define BATT_LEVEL2_VOLT	3724
#define BATT_LEVEL1_VOLT	3675
#define BATT_LEVEL0_VOLT	3555
...
#elif defined CONFIG_INPUT_GPIOKEY_MATRIX
...
static const unsigned short newKeymap[ARRAY_SIZE(bcm215xx_row_gpios) * ARRAY_SIZE(bcm215xx_col_gpios)] = {
	...
	[KEYMAP_INDEX(BCM_KEY_ROW_6, BCM_KEY_COL_0)] = KEY_HOMEPAGE,
...
struct i2c_slave_platform_data luisa_ds_cam_pdata = {
		.i2c_spd = I2C_SPD_400K,
};

static struct i2c_board_info __initdata bcm21553_cam_i2c_board_info[] = {
        {I2C_BOARD_INFO("cami2c_main", 0x1a),
         .platform_data = (void *)&luisa_cam_pdata,
         .irq = IRQ_CAM_CCP2, 
         },
	{I2C_BOARD_INFO("cami2c_sub", 0x60 >> 1),
	 .platform_data = (void *)&luisa_ds_cam_pdata,
	 .irq = IRQ_CAM_CCP2, 
	 },
};
#endif
...
enum {
	...
	PMU_REG_0X2E_PM_A7OPMODCTRL = 0x2E,
        ...
        PMU_REG_0x3E_PM_ADISCHARG1 = 0x3E,
...
// KARL, to remove dldo3 from camera usage.
/*
...
//	static u16 bat_adc[] = {0x2BE, 0x2D3, 0x2E8, 0x2FC, 0x311, 0x32C, 0x364};
	...
	static u32 TempAdcTable[] = {     866, 774,717,656,606,533,481,415,370,319,281,234,199,170,122,111,103,97,}; 
	static u32 TempDegreeTable[] ={-20, -10,  -5,    0,   5,  10,  15, 20,  25,  30,  35, 40,  45,  50,  60,  63, 65,67,};
...
{
	max8986_load_sysparm(PMU_REG_0x3E_PM_ADISCHARG1,
			     MAX8986_PM_REG_ADISCHARGE1, max8986);
...
#if 1		// Albert.Chung_2011-09-01 : CSP#451281
                        /*
                            Signal Name           GPIO #    (Pin Name)              IOCR                              Description
                            --------------------------------------------------------------------
                             CAM_3M_RST         GPIO63 (DIGMICDATA)           IOCR0[24]                      3M Camera Reset
                             CAM_3M_STBY       GPIO55 (GPEN10)                   IOCR0[16,15]                 3M Camera Stand-by
                            --------------------------------------------------------------------
                             VGA_CAM_D_EN     GPIO52 (GPEN7)                    IOCR0[12]                        Control pin for VGA Camera Digital Logic Power 
                             CAM_VGA_RST        GPIO42 (ANA_SYSCLKEN)      IOCR2[18]                        VGA Camera Reset
                             CAM_VGA_STBY      GPIO53 (GPEN8)                    IOCR2[21],IOCR0[13]       VGA Camera Stand-by
                            --------------------------------------------------------------------
                             CAM_IO_EN           GPIO23 (GPIO23)                                                            Control pin for Digital I/O power for 3M/VGA Camera

                             # Caution : GPEN8 is configured to GPIO53 by combination of IOCR2[21]=1 and IOCR0[13]=1
                        */
                        // Pin Muxing Configuratin : GPIO 63/55/53/52 : IOCR0[24,16-15,13,12]
                        val = readl(ADDR_SYSCFG_IOCR0);
                        val |= (SYSCFG_IOCR0_DIGMIC_GPIO6362_MUX | SYSCFG_IOCR0_GPIO55_GPEN10_MUX(0x3) | SYSCFG_IOCR0_GPIO53_GPEN8_MUX | SYSCFG_IOCR0_GPIO52_GPEN7_MUX);
                        writel(val, ADDR_SYSCFG_IOCR0);

                        // Pin Muxing Configuratin : GPIO42/GPIO53 : IOCR2[21,18]
                        writel(readl(ADDR_SYSCFG_IOCR2)|((1<< 21) | (1<<18)) , ADDR_SYSCFG_IOCR2);

                        // Pin Muxing Configuratin : GPIO23 : IOCR6[6]
                        val = readl(ADDR_SYSCFG_IOCR6);
                        val &= ~(1<<6);
                        writel(val, ADDR_SYSCFG_IOCR6);

                        // GPIO direction (GPIO63/55/53/52) : Output
                        val = readl(ADDR_GPIO_IOTR3);
                        val &= ~((3<<30) | (3<<14) | (3<<10) | (3<<8)); 
                        val |= ((2<<30) | (2<<14) | (2<<10) | (2<<8));  //direction : output
                        writel(val, ADDR_GPIO_IOTR3); 

                        // GPIO direction (GPIO42) : Output
                        val = readl(ADDR_GPIO_IOTR2);
                        val &= ~(3<<20); 
                        val |= (2<<20);  //direction : output
                        writel(val, ADDR_GPIO_IOTR2);

                        // GPIO direction (GPIO23) : Output
                        val = readl(ADDR_GPIO_IOTR1);
                        val &= ~(3<<14); 
                        val |= (2<<14);  //direction : output
                        writel(val, ADDR_GPIO_IOTR1);      

                        // Enable pull-up/down (0 : Disable pull up/down,   1 : Enable pull up/down)
                        writel(readl(ADDR_GPIO_GPIPEN0)|(1<<23), ADDR_GPIO_GPIPEN0);           // GPIO23
                        writel(readl(ADDR_GPIO_GPIPEN1)|((1<<31)|(1<<23)|(1<<21)|(1<<20)|(1<<10)), ADDR_GPIO_GPIPEN1);     // (GPIO63/55/53/52/42)
                        
                        // Pull-down (0 : Pull down,  1 : Pull up)
                        writel(readl(ADDR_GPIO_GPIPUD0)&~(1<<23), ADDR_GPIO_GPIPUD0);           // GPIO23
                        writel(readl(ADDR_GPIO_GPIPUD1)&~((1<<31)|(1<<23)|(1<<21)|(1<<20)|(1<<10)), ADDR_GPIO_GPIPUD1);     // (GPIO63/55/53/52/42)
                        
                        // Set GPIOs low (0 : Low,   1 : High)
                        writel(readl(ADDR_GPIO_GPOR0)&~(1<<23), ADDR_GPIO_GPOR0);           // GPIO23
                        writel(readl(ADDR_GPIO_GPOR1)&~((1<<31)|(1<<23)|(1<<21)|(1<<20)|(1<<10)), ADDR_GPIO_GPOR1);     // (GPIO63/55/53/52/42)
#else
	...
	case SYSCFG_SENSORS:
		if(op == SYSCFG_ENABLE){
			...
			writel(readl(ADDR_GPIO_IOTR1) & ~(0x3 <<( SENSOR_SDA-16)*2 )  | (0x2 << ( SENSOR_SDA-16)*2) , ADDR_GPIO_IOTR1);
		        ...
		        bcm_gpio_pull_up_down_enable(SENSOR_SCL, false);
		        bcm_gpio_pull_up_down_enable(SENSOR_SDA, false);
...
#ifdef CONFIG_INPUT_GPIOKEY_MATRIX
