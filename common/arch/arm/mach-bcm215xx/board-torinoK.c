...
*
*	@file	arch/arm/mach-bcm215xx/board-torinoK.c
*
...
static void Bcm_FUEL_GpioInit_Setting(void)
{
	...
	//gpio_direction_input(FG_SCL);
	bcm_gpio_pull_up(FG_SCL, true);	
	bcm_gpio_pull_up(FG_SDA, true);
	
	bcm_gpio_pull_up_down_enable(FG_SCL, true);
	bcm_gpio_pull_up_down_enable(FG_SDA, true);
	...
	if (gpio_request(GP_OUT, "fuel_irq") == 0) {
		...
		set_irq_type(GPIO_TO_IRQ(GP_OUT), IRQF_TRIGGER_FALLING);
	} else {
...
static void torinoK_init_gpio(void)
{
/* +++ H/W req */
#define ADDR_GPIO_GPIPUD0 (HW_GPIO_BASE + 0x028) //0x088CE028 GPIO 0 - 31
#define ADDR_GPIO_GPIPUD1 (HW_GPIO_BASE + 0x02c) //0x088CE02C GPIO 32 - 63
