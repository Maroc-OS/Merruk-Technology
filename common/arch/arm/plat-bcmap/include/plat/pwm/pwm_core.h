...
struct pwm_channel_ops {
	...
	int (*request) (struct pwm_channel *chan);
	/**< PWM channel request callback for the pwm driver */
	void (*free) (struct pwm_channel *chan);
	/**< Callback to free the pwm channel */
	int (*config) (struct pwm_channel *chan, int duty_ns, int period_ns);
	/**< Callback to configure the pwm channel */
	int (*enable) (struct pwm_channel *chan);
	/**< Callback enable the pwm channel */
	void (*disable) (struct pwm_channel *chan);
	/**< Callback to disable the pwm channel */
	int (*setmode) (struct pwm_channel *chan, int mode);
	/**< Callback to set the mode of pwm channel */
};
 ...
 */
struct pwm_desc {
	int pwm_id; /**< Id and name for a pwm channel. the id must
	* have a value from 0 to MAX_PWM_DEVICE_ID.
	*/
