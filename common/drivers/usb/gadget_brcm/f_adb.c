 ...
 */
 ...
 /*******************************************************************************************
Copyright [DATE] Broadcom Corporation.  All rights reserved.
...
void adb_function_enable(int enable)
{
	...
	pr_err("adb_function_enable+++\n");
	if (dev) {
		DBG(dev->cdev, "adb_function_enable(%s)\n",
			enable ? "true" : "false");
		pr_err("adb_function_enable(%s)\n",enable ? "true" : "false");
 ...
 *
 */
void adb_interface_enable(int enable)
{
	struct adb_dev *dev = _adb_dev;

	pr_info("adb_interface_enable+++\n");

	if (dev) {		
		pr_info("adb_interface_enable(%s)\n",enable ? "true" : "false");		
		set_usb_interface(&dev->function, adb_interface_desc.bInterfaceNumber, enable);

		/* If it is enabled, it needs to be disabled before software disconnects the USB devices (Android_force_reenum).... */
		if (enable)
			adb_function_disable(&dev->function);
	}
	pr_info("adb_interface_enable---\n");
}
