...
static short app_mode;
static short idle_count;

static atomic_t suspend_flag = ATOMIC_INIT(0);

static struct akm8975_platform_data *pdata;
static struct timer_list akm_flag_timer;
...
static int AKECS_GetData(char *rbuf, int size)
{
	...
	failure_count = 0;
	msleep(100); /* TODO: why is this required? */
	return 0;
}
...
{
	wait_event_interruptible_timeout(open_wq,
...
void akm_flag_timer_func(unsigned long data)
{
	AKMFUNC("akm_flag_timer_func");
	app_mode = AK8975_APP_IDLE_MODE;
			  ...
			  unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		...
		case ECS_IOCTL_APP_SET_APP_MODE:
			if (copy_from_user(&flag, argp, sizeof(flag))) {
				return -EFAULT;
			}
			break;
	...
	switch (cmd) {
		...
		case ECS_IOCTL_APP_SET_APP_MODE:
			app_mode = flag;
			break;
		case ECS_IOCTL_APP_GET_APP_MODE:
			flag = app_mode;
			break;
		default:
			return -ENOTTY;
	...
	switch (cmd) {
		...
		case ECS_IOCTL_APP_SET_MVFLAG:
			if (atomic_read(&m_flag) || atomic_read(&mv_flag)) {
				if (timer_pending(&akm_flag_timer))
					del_timer(&akm_flag_timer);
				app_mode = AK8975_APP_MEASURE_MODE;
				idle_count %= 255;
				input_report_abs(data->input_dev, ABS_MISC, idle_count++);
			} else {
				/* 5 sec expiry */
				if (!timer_pending(&akm_flag_timer))
					mod_timer(&akm_flag_timer, jiffies + 5 * HZ);
			}
			break;
		case ECS_IOCTL_APP_GET_MFLAG:
		   ...
		   unsigned long arg)
{
	...
	switch (cmd) {
		case ECS_IOCTL_WRITE:
			...
			AKMDBG("invalid argument. out of bounds");
				return -EINVAL;
			}
		...
		case ECS_IOCTL_GETDATA:
			...
			break;
		case ECS_IOCTL_GET_OFFSET:
			AKMFUNC("IOCTL_GET_OFFSET");
			offset[0] = pdata->pre_offsetx;
			offset[1] = pdata->pre_offsety;
			offset[2] = pdata->pre_offsetz;
			break;
		...
		case ECS_IOCTL_GET_DELAY:
			...
			delay = akmd_delay;
			break;
		case ECS_IOCTL_GET_APP_MODE:
			if (atomic_read(&suspend_flag))
			appmode = AK8975_APP_IDLE_MODE;
		else
			appmode = app_mode;
			break;
		default:
	...
	switch (cmd) {
		...
		case ECS_IOCTL_GETDATA:
			...
			break;
		case ECS_IOCTL_GET_OFFSET:
			if (copy_to_user(argp, &offset, sizeof(offset))) {
				AKMDBG("copy_to_user failed.");
				return -EFAULT;
			}
			break;
		...
		case ECS_IOCTL_GET_DELAY:
			...
			break;
		case ECS_IOCTL_GET_APP_MODE:
			if (copy_to_user(argp, &appmode, sizeof(appmode))) {
				AKMDBG("copy_to_user failed.");
				return -EFAULT;
			}
			break;
...
int akm8975_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	err = request_irq(client->irq, akm8975_interrupt, IRQ_TYPE_EDGE_RISING,
					  "akm8975_DRDY", akm);
	if (err < 0) {
		printk(KERN_ERR "AKM8975 akm8975_probe: request irq failed for %d with error %d\n", client->irq, err);
		goto exit4;
	}
	...
	/* event to signal mode change 0 to ff */
	input_set_abs_params(akm->input_dev, ABS_MISC, 0, 255, 0, 0);
	/* Set name */
	...
	setup_timer(&akm_flag_timer, akm_flag_timer_func, 0);
#ifdef CONFIG_HAS_EARLYSUSPEND
