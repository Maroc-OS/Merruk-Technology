...
static long ATC_KERNEL_Ioctl(struct file *filp, unsigned int cmd, UInt32 arg )
{
    ...
    switch( cmd )
    {
		...
		case ATC_KERNEL_Reboot_System:
			ATC_KERNEL_TRACE(( "ATC_KERNEL_Ioctl() - ATC_KERNEL_Reboot_System\n" ));
			arm_machine_restart(0, "reboot");
			break;
...
static int __init ATC_KERNEL_ModuleInit(void)
{
    ...
    {
        // AP only, so don't continue with ATC init
        ATC_KERNEL_TRACE(( "ATC_KERNEL_ModuleInit: AP only; don't register driver\n"));
        return 0;
    }
