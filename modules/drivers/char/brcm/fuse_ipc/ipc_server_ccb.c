...
/**
   Initially register all endpoint callbacks with the server's default callbacks.
...
{
  { IPC_SERVER_NAME_STR, IPC_EP_Capi2App,        ipcs_capi2app_fc,  ipcs_capi2app_bd,			IPC_EP_HDR_SZ, NULL, EIpcs},
  { IPC_SERVER_NAME_STR, IPC_EP_PsAppData,    ipcs_capi2psd_fc,  ipcs_capi2psd_bd,			IPC_EP_HDR_SZ, NULL, EIpcs},
  { IPC_SERVER_NAME_STR, IPC_EP_LogApps,         NULL,				ipcs_cplog_bd,				IPC_EP_HDR_SZ, NULL, EIpcs},
  { IPC_SERVER_NAME_STR, IPC_EP_AudioControl_AP, ipcs_audioctrl_fc, ipcs_audioctrl_bd,			IPC_EP_HDR_SZ, NULL, EIpcs},
  { IPC_SERVER_NAME_STR, IPC_EP_CsdAppCSDData, ipcs_capi2csd_fc,  ipcs_capi2csd_bf,			IPC_EP_HDR_SZ, NULL, EIpcs}   
};
...
void ipcs_capi2psd_bd(IPC_Buffer ipc_buffer)
{
   ...
   {
      (*g_clnt_info_tbl[EpCapi2AppData].buffer_delivery_cb)(ipc_buffer);
   }
   else
   {
   	IPC_FreeBuffer(ipc_buffer);
