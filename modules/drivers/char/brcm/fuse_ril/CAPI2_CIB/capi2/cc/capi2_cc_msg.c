...
bool_t     xdr_CcCnapName_t(XDR *xdrs, CcCnapName_t *args)
{
	XDR_LOG(xdrs,"CcCnapName_t ")


	if( 
		XDR_ENUM(xdrs,  &(args->nameCoding), ALPHA_CODING_t) && 
		xdr_opaque(xdrs, (caddr_t) args->name, CC_MAX_CNAP_NAME_LEN) &&
...
static const struct xdr_discrim CC_FacilityInd_dscrm[3] = 
{
...
bool_t
xdr_CC_FacilityInd_t(XDR *xdrs, CC_FacilityInd_t *args)
{
	
	struct xdr_discrim *entry = NULL;
	enum_t dscm = args->type;
	
	XDR_LOG(xdrs,"CC_FacilityInd_t ")
			...
			)
		{
			...
			return( xdr_union(xdrs, (enum_t *)&dscm, (caddr_t)(void *)&(args->param),CC_FacilityInd_dscrm, NULL_xdrproc_t, &entry, NULL));
		}
		else
			return(FALSE);
}
