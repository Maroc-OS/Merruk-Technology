...
bool_t
xdr_USIM_PBK_CONFIG_t(XDR* xdrs, USIM_PBK_CONFIG_t* data)
{
	if ( XDR_ENUM(xdrs, &data->pbkId, USIM_PBK_ID_t) &&
		 _xdr_u_char(xdrs, &data->numOfTotalEntry, "numOfTotalEntry") &&
		 _xdr_u_char(xdrs, &data->entryLength, "entryLength") &&
		 _xdr_u_char(xdrs, &data->numOfUsedEntry, "numOfUsedEntry") )
	{
		return (TRUE);
	}
	else
...
bool_t  
xdr_USIM_PBK_CONFIG_LIST_t(XDR* xdrs, USIM_PBK_CONFIG_LIST_t* data)
{
	int i;

	XDR_LOG(xdrs, "USIM_PBK_CONFIG_LIST_t")

	for (i = 0; i < MAX_NUM_OF_ADN_SET; i++)
	{
		if ( !xdr_vector(xdrs, (char *)(void *) &data->configInfo[i], NUM_OF_USIM_PBK_ID, sizeof(USIM_PBK_CONFIG_t), (xdrproc_t)xdr_USIM_PBK_CONFIG_t) )
		{
			return (FALSE);
		}
	}
...
xdr_PBK_ENTRY_DATA_RSP_t(XDR *xdrs, PBK_ENTRY_DATA_RSP_t* rsp)
{
	...
	if( _xdr_u_int16_t(xdrs, &rsp->total_entries,"total_entries") &&
		...
		_xdr_u_int16_t(xdrs, &rsp->next_valid_index,"next_valid_index")
		)
		return(TRUE);
	else
		return(FALSE);
