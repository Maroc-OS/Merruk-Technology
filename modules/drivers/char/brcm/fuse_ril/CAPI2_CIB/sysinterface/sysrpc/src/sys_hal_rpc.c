...
Result_t Handle_CAPI2_HAL_EM_BATTMGR_Notification(RPC_Msg_t* pReqMsg, HAL_EM_BATTMGR_Events_en_t 	event, UInt16 inLevel, UInt16 inAdc_avg, UInt16 inTotal_levels)
{
	...
	else if (event == BATTMGR_RESTART_CHARGER && gChargerRestartCbk)
	{
		gChargerRestartCbk();
	}
	else if (event == BATTMGR_CHARGER_TIMEOUT && gBattLevelCbk)
	{
		gChargerTimeoutCbk();
	}
	else
	...
	)
{
	...
	else if (event == BATTMGR_RESTART_CHARGER)
	{
		gChargerRestartCbk = (HAL_EM_BATTMGR_Cb_t)callback;
	}	
	else if (event == BATTMGR_CHARGER_TIMEOUT)
	{
		gChargerTimeoutCbk = (HAL_EM_BATTMGR_Cb_t)callback;
	}
	else
	{
	}
...
static void sysrpc_battmgr_extreme_temp_cb(UInt16 inLevel, UInt16 inAdc_avg)
{
	CAPI2_HAL_EM_BATTMGR_Notification(0, SYS_GetClientId(), BATTMGR_BATT_EXTREME_TEMP_EVENT,inLevel,inAdc_avg,0);
}

static void sysrpc_battmgr_restart_charger_cb( void )
{
	CAPI2_HAL_EM_BATTMGR_Notification(0, SYS_GetClientId(), BATTMGR_RESTART_CHARGER,0,0,0);
}

static void sysrpc_battmgr_charger_timeout_cb( void )
{
	CAPI2_HAL_EM_BATTMGR_Notification(0, SYS_GetClientId(), BATTMGR_CHARGER_TIMEOUT,0,0,0);
}

#endif

/*There are 6 CB associated with the ADC on the COMM side. The rest are PMU specific and can be handled in the HAL_EM_BATTMGR_Proxy.
...
HAL_EM_BATTMGR_ErrorCode_en_t SYS_HAL_EM_BATTMGR_RegisterEventCB(HAL_EM_BATTMGR_Events_en_t event, Boolean validCbk)
{
	...
	{
#if (defined(COMM_EM))	
		errorCode =	HAL_EM_BATTMGR_RegisterEventCB(	BATTMGR_BATT_EXTREME_TEMP_EVENT, (validCbk)?(void*)sysrpc_battmgr_extreme_temp_cb:NULL );
#endif
	}
	else if (event == BATTMGR_RESTART_CHARGER)
	{
#if (defined(COMM_EM))	
		errorCode = HAL_EM_BATTMGR_RegisterEventCB( BATTMGR_RESTART_CHARGER, (validCbk)?(void*)sysrpc_battmgr_restart_charger_cb:NULL );
#endif
	}
	else if (event == BATTMGR_CHARGER_TIMEOUT)
	{
#if (defined(COMM_EM))	
		errorCode = HAL_EM_BATTMGR_RegisterEventCB( BATTMGR_CHARGER_TIMEOUT, (validCbk)?(void*)sysrpc_battmgr_charger_timeout_cb:NULL );
#endif
	}
	else
	{
