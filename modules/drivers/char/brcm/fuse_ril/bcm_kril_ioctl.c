...
int KRIL_DevSpecific_Cmd(unsigned short client , UInt32 CmdID, void *data, size_t datalen)
{
    ...
    if ( (client == BCM_POWER_CLIENT) && (RIL_DEVSPECIFICPARAM_BCM_PMU_GET_BATT_ADC == CmdID) ) 
    {
        // temporary special case because PMU issues this request before KRIL driver
        // has been opened by URIL... This will go away soon, as sysrpc will be made into
        // a standalone module with a separate interface for PMU
        PMU_BattADCReq();
        return 1;
    }
