...
typedef struct
{
	...
	UInt8 clientID;
	RPC_InitParams_t clientParams2;
	RPC_InitParams_t SyncRpcParams2;
	RPC_SyncInitParams_t syncInitParams2;
}RPC_SyncParams_t;
...
RPC_Handle_t RPC_SyncRegisterClient(RPC_InitParams_t*	initParams, RPC_SyncInitParams_t* syncInitParams)
{
	...
	internalParam->clientParams2 = *initParams;
	internalParam->SyncRpcParams2 = *initParams;
	...
	internalParam->SyncRpcParams2.respCb = RPC_SyncHandleResponse;
        internalParam->SyncRpcParams2.ackCb = RPC_SyncHandleAck;
        internalParam->SyncRpcParams2.userData = (UInt32)internalParam;

	internalParam->syncInitParams = *syncInitParams;
	internalParam->syncInitParams2 = *syncInitParams;
							...
							UInt32 userContextData)
{
	...
	if(internalParam && internalParam->syncInitParams.copyCb && (!(((unsigned long)(internalParam->syncInitParams.copyCb)) & (unsigned long)(0x3))))
		...
		if(internalParam && internalParam->syncInitParams.copyCb)
			printk("RPC_SyncHandleResponse: syncInitParams.copyCb corrupted: %x!\n",internalParam->syncInitParams.copyCb);
		if(internalParam && internalParam->syncInitParams2.copyCb && (!(((unsigned long)(internalParam->syncInitParams2.copyCb)) & (unsigned long)(0x3))))
			internalParam->syncInitParams2.copyCb(msgType, dataBuf, (taskMap)?taskMap->dataLen:0, (taskMap)?taskMap->data:NULL, &dataLength,  &result);
		else {
			if(internalParam && internalParam->syncInitParams2.copyCb){
				printk("RPC_SyncHandleResponse: syncInitParams.copyCb2 corrupted too: %x!\n",internalParam->syncInitParams2.copyCb);
				BUG();
			}
		}
	}
