...
int VSP_Init(void)
{
    if (RPC_RESULT_OK != RPC_PACKET_RegisterDataInd(g_CsdClientId, INTERFACE_CSD, CSD_DataIndCB, CSD_FlowCtrlCB))
    {
...
static int __init bcm_fuse_vsp_init_module(void)
{
    ...
    {
        VSP_DEBUG(DBG_INFO, "bcm_fuse_vsp_init_module: AP only; don't register driver\n");
        return ret;
    }
