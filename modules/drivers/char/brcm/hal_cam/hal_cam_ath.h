...
typedef struct {
    ...
    UInt32						intf_mode;				///< Sensor Interface Mode
    UInt32						intf_port;				///< Sensor Interface Physical Port
...
typedef struct camdrv_FocusControl_s {
    CamFocusControlMode_t	default_setting;	///< default=CamFocusControlOff:  Values allowed  CamFocusControlMode_t
    CamFocusControlMode_t	cur_setting;		///< CamFocusControlMode_t
    UInt32					settings;			///< Settings Allowed: CamFocusControlMode_t bit masked
} CAMDRV_FOCUSCONTROL_S;

typedef struct camdrv_DigitalZoomMode_s {
    CamZoom_t	default_setting;			///< default=CamZoom_1_0:  Values allowed  CamZoom_t
    CamZoom_t	cur_setting;				///< CamZoom_t
    CamZoom_t	max_zoom;					///< Max Zoom parameter Allowed (256/max_zoom = *zoom) or 0 < CamZoom_t < 32 if Table look-up
    Boolean			capable;					///< Sensor capable: TRUE/FALSE:
} CAMDRV_DIGITALZOOMMODE_S;
...
/***************************************************************************
*
*  List of camera transformations possible
*
*****************************************************************************/
typedef enum
{
    CAMDRV_TRANSFORM_NONE      = 0,
    CAMDRV_TRANSFORM_HFLIP     = (1<<0),
    CAMDRV_TRANSFORM_VFLIP     = (1<<1),
    CAMDRV_TRANSFORM_TRANSPOSE = (1<<2)
} CAMDRV_TRANSFORM_T;
...
typedef enum
{
    CAMDRV_IMAGE_MIN     = 0, //bounds for error checking
    
    CAMDRV_IMAGE_RGB565          = (1<<0),          /* Bayer image, extra defines which variant is being used */
    CAMDRV_IMAGE_RGB444         = (1<<1),
    CAMDRV_IMAGE_YUV422         = (1<<2),
    CAMDRV_IMAGE_YUV420         = (1<<5),
    CAMDRV_IMAGE_YUV420_SEMI    = (1<<6),
    CAMDRV_IMAGE_YUV420_PLANAR  = (1<<7),
    CAMDRV_IMAGE_JPEG           = 0x8,          
    CAMDRV_IMAGE_BYPASS         = (1<<9),  ///< Bypass mode for Camera Controller 

    CAMDRV_IMAGE_MAX,            //bounds for error checking
} CAMDRV_IMAGE_TYPE_T;
...
typedef enum
{ 
    CAMDRV_IMAGE_ORDER_MIN      = 0,        //bounds for error checking
    
    CAMDRV_IMAGE_BAYER_RGGB     = (1<<0),
    CAMDRV_IMAGE_BAYER_GBRG     = (1<<1),
    CAMDRV_IMAGE_BAYER_BGGR     = (1<<2),
    CAMDRV_IMAGE_BAYER_GRBG     = (1<<3),
    CAMDRV_IMAGE_YUV422_YCbYCr  = (1<<4),
    CAMDRV_IMAGE_YUV422_YCrYCb  = (1<<5),
    CAMDRV_IMAGE_YUV422_CbYCrY  = (1<<6),
    CAMDRV_IMAGE_YUV422_CrYCbY  = (1<<7),
    CAMDRV_IMAGE_RGB_RGB        = (1<<8),
    CAMDRV_IMAGE_RGB_BGR        = (1<<9),

    CAMDRV_IMAGE_ORDER_MAX,                 //bounds for error checking
} CAMDRV_IMAGE_ORDER_T;
...
typedef enum
{
    CAMDRV_DATA_SIZE_NONE             = 0,             ///< No data present
    CAMDRV_DATA_SIZE_6BIT             = (1 << 0),      ///< 6 bit data
    CAMDRV_DATA_SIZE_7BIT             = (1 << 1),      ///< 7 bit data
    CAMDRV_DATA_SIZE_8BIT             = (1 << 2),      ///< 8 bit data
    CAMDRV_DATA_SIZE_10BIT            = (1 << 3),      ///< 10 bit data
    CAMDRV_DATA_SIZE_12BIT            = (1 << 4),      ///< 12 bit data
    CAMDRV_DATA_SIZE_14BIT            = (1 << 5),      ///< 14 bit data
    CAMDRV_DATA_SIZE_16BIT            = (1 << 6),      ///< 16 bit data
} CAMDRV_DATA_SIZE_T;

/***************************************************************************
*
*  The encoding that the camera IF transciever (eg CCP2) should perform on the data before writing to memory.
*
*****************************************************************************/
typedef enum
{
    CAMDRV_ENCODE_NONE,
    CAMDRV_ENCODE_DPCM_10to8,
    CAMDRV_ENCODE_DPCM_12to8,
    CAMDRV_ENCODE_DPCM_14to8
} CAMDRV_ENCODE_T;

/***************************************************************************
*
*  The decoding that the camera IF transciever (eg CCP2) should perform on the data read from camera interface
*
*****************************************************************************/
typedef enum
{
   CAMDRV_DECODE_NONE,
   CAMDRV_DECODE_DPCM_8to10,
   CAMDRV_DECODE_DPCM_8to12,
} CAMDRV_DECODE_T;
...
typedef enum 
{
    CAMDRV_MODE_VIDEO           = (1<<0),       // Single Pipe
    CAMDRV_MODE_STILL           = (1<<1),       // Single Pipe	//call pp zoom function		
    CAMDRV_MODE_VIDEO_THUMB     = (1<<2),       // 2 Pipes
    CAMDRV_MODE_STILL_THUMB     = (1<<3),       // 2 Pipes
	CAMDRV_MODE_STILL_PPZOOM	= (1<<4),
	CAMDRV_MODE_IDLE			= (1<<5),
	CAMDRV_MODE_SELECT			= (1<<6)
} CAMDRV_CAPTURE_MODE_T;
...
typedef enum
{
    CAMDRV_RESOLUTION_SQCIF      = 0x1,     ///< image size 128x96
    CAMDRV_RESOLUTION_QQVGA      = 0x2,     ///< image size 160x120
    CAMDRV_RESOLUTION_QCIF       = 0x4,     ///< image size 176x144
    CAMDRV_RESOLUTION_240x180    = 0x8,     ///< image size 240x180
    CAMDRV_RESOLUTION_R_QVGA     = 0x10,     ///< image size 240x320
    CAMDRV_RESOLUTION_QVGA       = 0x20,     ///< image size 320x240
    CAMDRV_RESOLUTION_CIF        = 0x40,     ///< image size 352x288
    CAMDRV_RESOLUTION_320x480    = 0x80,     ///< image size 320x480
    CAMDRV_RESOLUTION_426x320    = 0x200,     ///< image size 426x320
    CAMDRV_RESOLUTION_640x240    = 0x100,     ///< image size 640x240
    CAMDRV_RESOLUTION_VGA        = 0x400,    ///< image size 640x480
    CAMDRV_RESOLUTION_SVGA       = 0x800,    ///< image size 800x600    
    CAMDRV_RESOLUTION_XGA        = 0x1000,    ///< image size 1024x768
    CAMDRV_RESOLUTION_1280x960   = 0x2000,    ///< image size 1280x960
    CAMDRV_RESOLUTION_SXGA       = 0x4000,    ///< image size 1280x1024
    CAMDRV_RESOLUTION_UXGA       = 0x8000,    ///< image size 1600x1200
//    CAMDRV_RESOLUTION_2048x1232  = (1 << 17),    ///< image size 2048x1232
    CAMDRV_RESOLUTION_QXGA       = 0x10000,    ///< image size 3M  
    CAMDRV_RESOLUTION_QSXGA      = 0x20000,    ///< image size 5M
//    CAMDRV_RESOLUTION_8MEG       = (1 << 20),    ///< image size 8M
    CAMDRV_RESOLUTION_480x360    = 0x40000,
    CAMDRV_RESOLUTION_INVALID	 = 0x80000	 ///< invalid image capture size 
} CAMDRV_RESOLUTION_T;

/***************************************************************************
*
*  The data input clocking mode for camera IF transciever.
*
*****************************************************************************/
typedef enum
{
   CAMDRV_CLOCKING_UNDEFINED = 0,     
   CAMDRV_CLOCKING_STROBE    = (1<<0),     
   CAMDRV_CLOCKING_CLOCK     = (1<<1),
   CAMDRV_CLOCKING_1LANE     = (1<<2),
   CAMDRV_CLOCKING_2LANES    = (1<<3),
} CAMDRV_CLOCKING_MODE_T;

typedef enum
{
    CAMDRV_SENSOR_MIN = 0,		///<  Min Number
    CAMDRV_SENSOR_PRIMARY,		///< Select Primary Camera Sensor
    CAMDRV_SENSOR_SECONDARY,	///< Select Secondary Camera Sensor
    CAMDRV_SENSOR_3,		    ///< Select 3rd Camera Sensor
    CAMDRV_SENSOR_MAX,			///<  Max Number
} CAMDRV_SENSOR_SELECTION;

typedef struct 
{
	UInt16					width;					///< width of frame in pixels
    UInt16					height;					///< height of frame in pixels
    UInt32					data_size;              ///< Minimum amount of data sent by the camera
    UInt32				    framerate_lo_absolute;  ///< [u24.8 format] Minimum possible framerate u24.8 format
    UInt32				    framerate_hi_absolute;  ///< [u24.8 format] Maximum possible framerate

    CAMDRV_TRANSFORM_T		transform;				///< Possible transformations in this mode / user requested transformations
    CAMDRV_IMAGE_TYPE_T		format;					///< Image format of the frame.
    CAMDRV_IMAGE_ORDER_T	image_order;			///< Format pixel order in the frame.
    CAMDRV_DATA_SIZE_T	    image_data_size;		///< Packing mode of the data.

    CAMDRV_DECODE_T         periph_dec;             ///< The decoding that the VideoCore transciever (eg CCP2) should perform on the data after reception. 
    CAMDRV_ENCODE_T		    periph_enc;				///< The encoding that the camera IF transciever (eg CCP2) should perform on the data before writing to memory.
    UInt32					block_length;           ///< Block length for DPCM encoded data - specified by caller
    CAMDRV_DATA_SIZE_T	    embedded_data_size;	    ///< The embedded data size from the frame.
    CAMDRV_CAPTURE_MODE_T	flags;					///< A bitfield of flags that can be set on the mode.
    UInt32                  framerate;              ///< [u24.8 format] Framerate achievable in this mode / user requested framerate
    UInt8					mechanical_shutter;     ///< It is possible to use mechanical shutter in this mode (set by CDI as it depends on lens driver) / user requests this feature
	UInt32					pre_frame;				///< Frames to throw out for Stills/ViewFinder/Video capture 	

    const char *            name;                   ///< short text description of the mode, e.g. "720P" 
    CAMDRV_RESOLUTION_T     size;                   ///<  image resolution
    UInt16                  padding_right;          ///< additional non-data pixels to the right of the frame 
    UInt16                  padding_down;           ///< additional non-data pixels below the frame 
    UInt32                  line_length;            ///< duration of line in ns 
    UInt32                  frame_length;           ///< Minimum frame length in lines 
    UInt8                   interlace_fields;
    UInt8                   first_field;
	CAMDRV_CLOCKING_MODE_T  clocking_mode;          ///< The clocking method for the camera.  

    UInt16                  full_frame_width;       ///< The width of the full frame the camera can supply in this mode, without any cropping 
    UInt16                  full_frame_height;      ///< The height of the full frame the camera can supply in this mode, without any cropping 
    UInt16                  sensor_crop_left;       ///< The x crop offset of the part of the frame that we actually read out 
    UInt16                  sensor_crop_top;        ///< The y crop offset of the part of the frame that we actually read out 
    UInt16                  sensor_crop_right;      ///< The x crop offset from the right of the part of the frame that we actually read out 
    UInt16                  sensor_crop_bottom;     ///< The y crop offset from the bottom of the part of the frame that we actually read out 
    UInt16                  scale_h;                ///< [8p8 format] The horizontal scaling applied to this image versus the native resolution of the sensor 
    UInt16                  scale_v;                ///< [8p8 format] The vertical scaling applied to this image versus the native resolution of the sensor 

    UInt32                  min_zoom_factor;        ///< The minimum zoom factor available to this mode 
    UInt32                  max_zoom_factor;        ///< The maximum zoom factor available to this mode 
    const char              *prev_zoom_mode;        ///< The previous zoom camera mode 
    const char              *next_zoom_mode;        ///< The next zoom camrea mode 

    //CamCaptureCB_t          cb;                     ///< Line Count callback (if needed, set by controlling layer)
    UInt32                  line_count;             ///< Line Count to invoke callback (if needed, set by controlling layer)
    void                    *pEmbeddedBuffer;       ///< Embedded buffer pointer 
} CAMDRV_DATA_MODE_S;


/** Sensor Configuration and Capabilities */
typedef struct {
	CAMDRV_DATA_MODE_S			video_mode;				///< Sensor Output capabilities
    CAMDRV_DATA_MODE_S			stills_mode;			///< Sensor Output capabilities
    CAMDRV_FOCUSCONTROL_S       focus_control_st;		///< Focus Settings & Capabilities
    CAMDRV_DIGITALZOOMMODE_S    digital_zoom_st;		///< Digital Zoom Settings & Capabilities
    CAMDRV_ESD_S                esd_st;					///< Sensor ESD Settings & Capabilities
    UInt32						intf_mode;				///< Sensor Interface Mode
    UInt32						intf_port;				///< Sensor Interface Physical Port
    UInt8 *                     pSensor_version;		///< Sensor version string
} HAL_CAM_ConfigCaps_st_t;
