...
static unsigned int Major=DMAMAJOR,ChanObtain=0,intCnt=0;
...
static void cleanup()
{
	...
	for(i=0;i<4;i++)
	{
		...
		{
			dma_free_coherent(NULL, num_transfers*transfer_size, csrcBuf[i].virtPtr, csrcBuf[i].physPtr);
			csrcBuf[i].virtPtr=NULL;
		}
		if(cdstBuf[i].virtPtr!=NULL)
		{
			dma_free_coherent(NULL, num_transfers*transfer_size, cdstBuf[i].virtPtr, cdstBuf[i].physPtr);
			cdstBuf[i].virtPtr=NULL;
		}
	}
}
...
static int test_dma_mem_to_mem()
{
    ...
    if (wait_for_completion_timeout(&dma_complete, msecs_to_jiffies(100)) <= 0)
    {
         pr_info("Timeout\n");
         cleanup();
         return -1;
	...
	cleanup(); 
	printk("T1:sec=%d nsec=%d    T2:sec=%d nsec=%d   Size:%dKB Time:%dmsec \n",t1.tv.sec,t1.tv.nsec,t2.tv.sec,t2.tv.nsec,(num_transfers*transfer_size)/1024,(t2.tv.nsec-t1.tv.nsec)/1000000);
	return 0;
}
//******************************************************************************
// 
...
static int test_dma_BufferList()
{
		...
		//Configure the channel
		dmaChInfoMem.priority = 1; //Change the priority
		DMA_Configure_ChanInfo();
		if (OSDAL_DMA_Config_Channel(gDmaChannel, &dmaChInfoMem)!= OSDAL_ERR_OK)
		{
			pr_info("---->DMADRV_Config_Channel - Failed for channel %d \n",gDmaChannel);
...
static int test_dma_MultipleLLI()
{
		...
		//Configure the channel
		dmaChInfoMem.priority = 0;//Change the priority
		DMA_Configure_ChanInfo();
		if (OSDAL_DMA_Config_Channel(gDmaChannel, &dmaChInfoMem)!= OSDAL_ERR_OK)
		{
			pr_info("---->DMADRV_Config_Channel - Failed for channel %d \n",gDmaChannel);
...
static int test_dma_Circular()
{
	...
	for(i=0;i<4;i++)
	{
		csrcBuf[i].virtPtr = dma_alloc_coherent(NULL, num_transfers*transfer_size, &csrcBuf[i].physPtr, GFP_KERNEL);
		...
		cdstBuf[i].virtPtr = dma_alloc_coherent(NULL, num_transfers*transfer_size, &cdstBuf[i].physPtr, GFP_KERNEL);
	...
	for(i=0;i<4;i++)
	{
		...
		for (j=0;j<(num_transfers*transfer_size)/SOURCE_WIDTH;j++)
		{
	...
	//Configure the channel
	dmaChInfoMem.priority = 0; //Chagne the priority
	DMA_Configure_ChanInfo();
	...
	for(i=0;i<4;i++)
	{
		...
		cirBufList[i].buffers[0].length = num_transfers*transfer_size;
	...
	//Start the transfer
	intCnt=0;init_completion(&dma_complete);
	if (OSDAL_DMA_Start_Transfer(gDmaChannel) != OSDAL_ERR_OK)
	{
		pr_info("DMADRV_Start_Transfer - Failed for channel %d\n",gDmaChannel);
		cleanup();
	...
	/* Wait for transfer done*/
	if (wait_for_completion_timeout(&dma_complete, msecs_to_jiffies(160)) <= 0)
        {
