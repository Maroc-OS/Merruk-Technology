...
// DGS
//gary #include "../fsr/Inc/FSR.h"
//gary #include "../fsr/Inc/FSR_BML.h"
//gary #include "../fsr/Inc/FSR_LLD_4K_OneNAND.h"

//gary #include "../../arch/arm/mach-msm/smd_private.h"
//gary #include "../../arch/arm/mach-msm/proc_comm.h"
...
//gary #define MSM_A2M_INT(n) (MSM_CSR_BASE + 0x400 + (n) * 4)
...
//gary extern void *smem_alloc(unsigned, unsigned);

// hsil for cpufreq
//gary extern int cpufreq_direct_set_policy(unsigned int cpu, const char *buf);

//Get charging status & charger Connect value!!!
//gary extern void get_charger_type(void);
//gary extern void msm_batt_check_event(void);
//gary extern int get_charging_status(void);
...
//gary 	writel(1, MSM_A2M_INT(3));
	intc_trigger_softirq(IRQ_SSIPC_A2C);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	{
		printk("[HSIL] %s(%d)\n", __func__, __LINE__);
//gary		cpufreq_direct_set_policy(0, "performance");
	}	
	else if (value == 0)
	{
		printk("[HSIL] %s(%d)\n", __func__, __LINE__);
//gary		cpufreq_direct_set_policy(0, "ondemand");
	}
	else
...
static void dpram_phone_reset(void)
{
	...
	msleep(100);
//gary	smsm_reset_modem(SMSM_SYSTEM_DOWNLOAD);
...
//gary	writel(0xCCCC, MSM_SHARED_RAM_BASE + 0x30); 
	printk("[DPRAM] call msm_proc_comm_reset_modem_now func\n");
//gary	msm_proc_comm_reset_modem_now();
}
...
//gary	get_charger_type();
//gary	get_charging_status();
//gary	msm_batt_check_event();
...
dprintk("register_dpram_driver: enter\n");
...
static int register_interrupt_handler(void)
{
	...
	/* @LDK@ dpram interrupt */
//gary	retval = request_irq(INT_A9_M2A_3, dpram_interrupt, IRQF_TRIGGER_RISING, DRIVER_NAME, NULL);
	retval = request_irq(IRQ_SSIPC_C2A, dpram_interrupt, IRQF_NO_SUSPEND, DRIVER_NAME, NULL);
...
static void check_miss_interrupt(void)
{
	...
	if (head != tail) 
	{
		...
		local_irq_save(flags);
//gary		dpram_interrupt(INT_A9_M2A_3, NULL);
		dpram_interrupt(IRQ_SSIPC_C2A, NULL);
		local_irq_restore(flags);
	}
...
static int __devinit dpram_probe(struct platform_device *dev)
{
	...
	SmemBase = (volatile unsigned char *)(ioremap_nocache(SSIPC_BASE, SSIPC_SIZE));
	if (!SmemBase)
	{
...
//gary        smem_flag->info = 0xAEAEAEAE;
//gary        msm_proc_comm_reset_modem_now();
}
...
//gary 	ent = create_proc_entry("silent", S_IRWXUGO, NULL);
