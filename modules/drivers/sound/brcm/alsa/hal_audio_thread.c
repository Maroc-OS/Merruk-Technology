...
static void AudioHalCtrlWorkThread(struct work_struct *work)
{
	...
	while(1)
	{
		...
		len = kfifo_out_locked(&sgThreadData.m_pkfifo, (unsigned char *)&msgHalAudioCtrl, sizeof(TMsgHalAudioCtrl), &sgThreadData.m_lock);
	...
	)
{
	...
	len = kfifo_in_locked(&sgThreadData.m_pkfifo, (unsigned char *)&msgHalAudioCtrl, sizeof(TMsgHalAudioCtrl), &sgThreadData.m_lock);
