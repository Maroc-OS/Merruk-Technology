...
static char *sgBrcm_auddrv_TestName[]={"Aud_play","Aud_Rec","Aud_control","Aud_voip"};
...
static struct device_attribute Brcm_auddrv_Test_attrib = __ATTR(BrcmAud_DrvTest, 0664,  Brcm_auddrv_TestSysfs_show, Brcm_auddrv_TestSysfs_store);
...
#define VOIP_FRAMES_IN_2_SEC	100
#define VOIP_FRAME_SIZE 320
#define VOIP_BUFFER_SIZE (VOIP_FRAME_SIZE*VOIP_FRAMES_IN_2_SEC)

static UInt8 VOIP_cap_buffer[VOIP_BUFFER_SIZE];

static UInt32 VOIP_buffer_count = 0;
static UInt32 VOIP_buf_ul_index = 0;
static UInt32 VOIP_buf_dl_index = 0;

//#define ENABLE_AUDVOC_ISR

#ifdef ENABLE_AUDVOC_ISR 
...
ssize_t Brcm_auddrv_TestSysfs_store(struct device *dev, struct device_attribute *attr,const char *buf, size_t count)
{
	...
	case 1: //Aud_play
       ...
       case 3: //Aud_control
        ...
        case 4:
        {
            DEBUG("I am in case 4 (VOIP) -- test type =%s arg1=%d, arg2=%d, arg3=%d, arg4=%d \n", sgBrcm_auddrv_TestName[sgBrcm_auddrv_TestValues[0]-1],
											sgBrcm_auddrv_TestValues[1],
											sgBrcm_auddrv_TestValues[2],
											sgBrcm_auddrv_TestValues[3],
                                                                             sgBrcm_auddrv_TestValues[4]);
...
)
{
    ...
    memcpy(&VOIP_cap_buffer[VOIP_buf_ul_index], pSrc, VOIP_FRAME_SIZE);
...
{
    VOIP_buffer_count++;
    if ( VOIP_buffer_count <= (VOIP_FRAMES_IN_2_SEC/4))
    {
        memset(pDst, 0, VOIP_FRAME_SIZE);

        //DEBUG(" VOIP_FillDL_CB :  VOIP_buf_dl_index silence 0x%x \r\n", VOIP_buf_dl_index);
    }
    else
    {
        //DEBUG(" VOIP_FillDL_CB :  VOIP_buf_dl_index 0x%x \r\n", VOIP_buf_dl_index);
        memcpy(pDst, &VOIP_cap_buffer[VOIP_buf_dl_index], VOIP_FRAME_SIZE);
...
void dump_audio_registers()
{
    ...
    DEBUG(MsgBuf);
     ...
     sprintf( MsgBuf, "IOCR0 =0x%08x,  IOCR2 =0x%08x ",
						*((UInt32 *) (SYSCFG_BASE_ADDR)),
						*((UInt32 *) (SYSCFG_BASE_ADDR+0x0C)));
...
//        DEBUG("clear_register for audio \n");
    ...
    // Fill the fifo
    chal_audioaopath_WriteFifo (NULL, (cUInt32 *)&hqAudioTestBuf[index], FIFO_FILL_SIZE);
