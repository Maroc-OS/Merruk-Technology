...
{
/**
The ASIC pin name is DIGMICCLK. The default output from this pin is DigMicClk (when 0x0888_0000[24] = ??.
When 0x0888_0000[24] =??the output from DIGMICCLK is GPIO62.

The power to the digital Mic is not supplied by the ASIC so it will not get affected by setting ASIC registers.

When 0x0888_0000[24] = ??you should get the MIC clk if you set 0x3080_0a80[5] = ??and
0x3080_0f58[6] = ??
*/
		if (on) 
		{
