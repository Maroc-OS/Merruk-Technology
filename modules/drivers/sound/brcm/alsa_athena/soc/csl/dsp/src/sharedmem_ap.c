...
static AP_SharedMem_t			AP_shared_mememory __attribute__((session("shared_rip_mem_sect")));
...
void SHAREDMEM_WriteDL_VoIP_Data(UInt16 codec_type, UInt16 *pSrc)
{
	...
	if (index > 4)
            Log_DebugPrintf(LOGID_AUDIO, "=====SHAREDMEM_WriteDL_VoIP_Data, invalid codec type!!!\r\n");
	else
		data_len = sVoIPDataLen[index];

        //Log_DebugPrintf(LOGID_AUDIO, "=====SHAREDMEM_WriteDL_VoIP_Data codec_type=0x%x dataLen=%d\r\n", codec_type, data_len);
...
UInt8 SHAREDMEM_ReadUL_VoIP_Data(UInt16 codec_type, UInt16 *pDst)
{
	...
	index = (codec_type & 0xf000) >> 12;
	if (index > 4)
            Log_DebugPrintf(LOGID_AUDIO, "=====SHAREDMEM_ReadUL_VoIP_Data, invalid codec type!!!\r\n");
	else
		data_len = sVoIPDataLen[index];

        // Log_DebugPrintf(LOGID_AUDIO, "=====SHAREDMEM_ReadUL_VoIP_Data codec_type=0x%x dataLen=%d\r\n", codec_type, data_len);
